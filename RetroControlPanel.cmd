...
::Check for updates
IF [%1]==[] (SETLOCAL ENABLEDELAYEDEXPANSION & PING -n 1 "raw.githubusercontent.com"|FINDSTR /r /c:"[0-9] *ms">nul & IF !errorlevel!==0 (ECHO. & ECHO Checking for updates... & BITSADMIN /transfer "RetroUpdater" /download /priority FOREGROUND "https://raw.githubusercontent.com/illsk1lls/RetroControlPanel/main/RetroControlPanel.cmd" "%~dp0update.dat">nul) & FC "%~f0" "%~dp0update.dat"|FIND "***">nul & IF !errorlevel!==0 (MOVE /Y "%~dp0update.dat" "%~f0">nul & ECHO @ECHO OFF ^& TIMEOUT 3 /NOBREAK ^>nul ^& START "" "%~f0" " %*" ^& ^(GOTO^) 2^>nul ^& DEL "%%~f0"^>nul ^& EXIT>"%~dp0relaunch.cmd" & CALL START "" "%~dp0relaunch.cmd" & EXIT) ELSE (DEL "%~dp0update.dat" /F /Q) & ENDLOCAL)
