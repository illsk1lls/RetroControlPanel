...
CD /D %~dp0 & IF NOT "%~f0" EQU "%ProgramData%\%~nx0" (COPY /Y "%~f0" "%ProgramData%">nul & START "" "%ProgramData%\%~nx0" %1& EXIT /b)
...
IF %errorlevel% == 12 (IF %1=="REVERT" (GOTO) 2>nul & DEL "%~f0">nul & EXIT
GOTO START