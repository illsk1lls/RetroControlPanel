@ECHO OFF
PING -n 1 "raw.githubusercontent.com"|FINDSTR /r /c:"[0-9] *ms">nul & IF %errorlevel% == 0 ECHO. & ECHO Checking for updates... & BITSADMIN /transfer "RetroUpdater" /download /priority FOREGROUND "https://raw.githubusercontent.com/illsk1lls/RetroControlPanel/main/RetroControlPanel.cmd" "%~dp0Update.Check">nul & FC "%~f0" "%~dp0Update.Check" | FIND "***">nul & (IF %errorlevel%==0 (MOVE /Y "%~dp0Update.Check" "%~f0">nul & START "" "%~f0" " %*" & EXIT /b) ELSE (ECHO. & ECHO You are running the latest version ;^)))
