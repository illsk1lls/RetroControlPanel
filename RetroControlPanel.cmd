...
TITLE %TitleName%
::Relaunch with admin rights if only if needed
...
::When (B)usiness mode is active the below Computer Rename/Domain Join option also adds the script to RunOnce. When (B)usiness mode is off it removes it from RunOnce. After domain join you are required to restart, this will allow you to finish setting up the user with; (i)nitial setup or (1) printers, etc...
...
::If (i) is pressed, InitialSetup, a script to setup new machines quickly, will be downloaded from github and run. Depending on whether you are in (B)usiness mode or not this will run different (slightly) software packages
